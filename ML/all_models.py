...
for i in range(200):
    ...
    svm_acc,svm_confusion,report = svm.find_accuracy(dataFrame)
    sum_svm_acc = sum_svm_acc+svm_acc
    sum_svm_confusion = np.add(sum_svm_confusion,svm_confusion)

    knn_acc,knn_confusion,report = knn.find_accuracy(dataFrame)
    sum_knn_acc = sum_knn_acc+knn_acc
    sum_knn_confusion = np.add(sum_knn_confusion,knn_confusion)

    dtree_acc,dtree_confusion,report = dtree.find_accuracy(dataFrame)
